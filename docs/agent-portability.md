...
| OpenCode | `.opencode/plugins/ponytail.mjs`, `.opencode/command/`, `hooks/`, `skills/` | Server plugin injects the ruleset each turn via `experimental.chat.system.transform`, adds it to non-primary agent prompts for sub-agent sessions, and persists `/ponytail` switches; reuses the shared instruction builder. |
